...
def run_module():
    ...
    rt_object_sql="SELECT RTO.name,RTO.label,RTO.asset_no,RTO.comment,RTD.dict_value FROM Object RTO, Dictionary RTD WHERE RTD.dict_key=RTO.objtype_id AND RTO.name='{}'".format(module.params['name'])
    rt_object={}
    ...
    if module.params['label'] == rt_object[1]:
        print("label matches")
    if module.params['assetnumber'] == rt_object[2]:
        print("assetnumber matches")
    if module.params['comment'] == rt_object[3]:
        print("comment matches")
    if module.params['type'] == rt_object[4]:
        print("type matches")
    ...
    if not props_match and not module.check_mode:
        with connection.cursor() as cursor:
            cursor.execute("SELECT dict_key FROM Dictionary WHERE dict_value='{}'".format(module.params['type']))
            objtype_id = cursor.fetchone()[0]
            if rt_object:
                cursor.execute("UPDATE Object SET name='{}', label='{}', objtype_id='{}', asset_no='{}', has_problems='no', comment='{}' WHERE name='{}';".format(module.params['name'],module.params['label'],objtype_id,module.params['assetnumber'],module.params['comment'],module.params['name']))
            if not rt_object:
                cursor.execute("INSERT INTO `Object` (name, label, objtype_id, asset_no, has_problems, comment) VALUES('{}', '{}', '{}', '{}', 'no', '{}');".format(module.params['name'],module.params['label'],objtype_id,module.params['assetnumber'],module.params['comment']))
        connection.commit()
        result['changed'] = True
